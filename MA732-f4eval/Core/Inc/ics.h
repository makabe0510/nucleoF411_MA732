...
// BOARD REVISION
#define REV_KJS01 0x00
#define REV_KJS02 0x01
#define REV_KJS03 0x02
#define REV_KJSNONE 0xFF
...
//static const uint8_t ADC_CHANNEL_ARRAY[4] = {0,6,1,2};//
...
// MAGENC CS PIN
#define MAGENC_CS_PORT GPIOC
#define MAGENC_CS_PIN GPIO_PIN_1
...
static const uint8_t MAGENC_EN = 0x01;
static const uint8_t MAGENC_NOT_EN = 0x00;
static const uint8_t MAGENC_NOT_EN_COUNT = 10;
static const uint8_t MAGENC_DELAY = 1;
...
//COMMUNICATION CONST
#define MAX_TXBUFF_LEN 2 + 2 * ADC_CHANNEL_NUM + 3 * PS_CHANNEL_NUM + 3 * GYRO_CHANNEL_NUM + 3 * ACC_CHANNEL_NUM + 2 * MAGENC_CHANNEL_NUM
...
struct servo_params {
	...
	uint8_t magenc_en;
	uint8_t magenc_cs;
	uint8_t magenc_errorFlag;
	uint8_t magenc_ocfFlag; // Avoid printing OCF flag everytime
	uint16_t magenc_position;
	uint16_t magenc_position_MA732;
	uint8_t magenc_position_raw[2];
	uint8_t magenc_debug;
	uint8_t magenc_noten_count;
	uint16_t MA732_BCT;
